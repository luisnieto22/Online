...
function admin_nombre_connected(){
	include "config.php";
	$id = $_SESSION['id'];
	$mysqli = connect();
...
function check_caducidad(){
	$mysqli = connect();
	$s = $mysqli->query("SELECT * FROM carro");
	while($r=mysqli_fetch_array($s)){
		$timeact = time();

		if($timeact>=$r['caducidad']){
			$mysqli->query("DELETE FROM carro WHERE id = '".$r['id']."'");
			$cant = $r['cant'];
			$mysqli->query("UPDATE productos SET instock = instock + $cant WHERE id = '".$r['id_producto']."'");
