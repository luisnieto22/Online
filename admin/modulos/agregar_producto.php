...
if(isset($enviar)){
	...
	$mysqli->query("INSERT INTO productos (name,precio,imagen,oferta,id_categoria,descargable,instock,id_vendedor) VALUES ('$name','$precio','$imagen','oferta','$categoria','$descargable','$instock','$id_vendedor')");
	alert("Producto agregado",1,'agregar_productos');
...
<form method="post" action="" enctype="multipart/form-data">
	...
	<div class="form-group">
		<label>Tiene algun archivo de descarga este producto?</label>
		<input class="form-control" type="file" name="descargable"/>
	</div>

	<!-- tomar cantidad en inventario -->
	
	<div class="form-group">
		<label>Cantidad en inventario</label>
		<input class="form-control" type="text" value="1" name="instock"/>
	</div>
...
<table class="table table-striped">
	...
	<?php
	if($id_vendedor>0){
		$prod = $mysqli->query("SELECT * FROM productos WHERE id_vendedor = '$id_vendedor'");
	}else{
		...
		while($rp=mysqli_fetch_array($prod)){
			...
			?>
				<tr>
					...
					<td><?=$categoria?></td>
					<td><b style="color:#f80"><?=$rp['instock']?></b> Disponibles</td>
					<td>
