...
if(isset($id) && isset($modificar)){
	...
	$ss = $mysqli->query("SELECT * FROM carro WHERE id = '$id'");
	$rr = mysqli_fetch_array($ss);

	$id_producto = $rr['id_producto'];


	$sc = $mysqli->query("SELECT * FROM productos WHERE id = '$id_producto'");
	$rc = mysqli_fetch_array($sc);

	if($modificar>$rc['instock']){
		alert("No tenemos suficientes productos en el inventario.",0,"carrito");
		die();
	}
...
if(isset($finalizar)){
	...
	while($r2=mysqli_fetch_array($q2)){
		...
		$monto = $rp['precio'];
		$mysqli->query("INSERT INTO productos_compra (id_compra,id_producto,cantidad,monto,id_vendedor) VALUES ('$ultima_compra','".$r2['id_producto']."','".$r2['cant']."','$monto','".$rp['id_vendedor']."')");
	}
...
<table class="table table-striped">
	<tr>
		...
		<th>Cantidad</th>
		<th style="color:#f80">Disponibles</th>
...
<?php
	...
	?>
		<tr>
			...
			<td><?=$cantidad?></td>
			<td style="color:#f80"><?=$r2['instock']?></td>
